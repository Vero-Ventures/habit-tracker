...
import {
  getAllCategory,
  getUserHabit,
  updateCustomUserHabit,
  updateUserHabit,
} from "../../store/ducks/habit";
...
const EditHabit = (props) => {
  ...
  const fetchAllData = async () => {
    ...
    getUserHabit(props.route.params.user_habit_id)
      .catch((err) => {
        Alert.alert(
          "Ops!",
      ...
      .then((res) => {
        if (res?.status === 200) {
          ...
          setFrequencyType(res.data.ush_frequency.type);
          setFrequencyTypeIos(res.data.ush_frequency.type);
          setCategory(res.data.habit.category.id);
          setCategoryIOS(res.data.habit.category);
          setHabitPhoto(res.data.habit.image);

          if (res.data.ush_frequency.type === "CUSTOM") {
            setFrequencyDays(res.data.ush_frequency.days);
          }

          if (res.data.ush_reminder_time) {
            setIsRemind(true);
            setTime(new Date(moment(res.data.ush_reminder_time, "HH:mm:ss")));
            setTimeIOS(
              new Date(moment(res.data.ush_reminder_time, "HH:mm:ss")),
            );
          }
  ...
  const updateHabitPreEstablished = async () => {
    ...
    updateUserHabit(props.route.params.user_habit_id, userHabit)
      .catch((err) => {
        Alert.alert(
          "Ops!",
          "Something went wrong with our servers. Please contact us.",
  ...
  const updateCustomHabit = async () => {
    ...
    let userHabit = {
      hab_name: name,
      hab_id_category: category,
      hab_description: habit_description,
      ush_frequency: { type: frequency_type },
    };
    ...
    habitForm.append("habit_id", props.route.params.user_habit_id);
    ...
    habitPhoto && image_changed
      ? habitForm.append("habit_image", habitPhoto)
      : null;

    updateCustomUserHabit(habitForm)
      .catch((err) => {
        setSending(false);
        Alert.alert(
          "Ops!",
