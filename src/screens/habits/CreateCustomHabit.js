...
const CreateCustomHabit = (props) => {
  ...
  useEffect(() => {
    if (props.route?.params?.habit) {
      setName(props.route.params.habit.habit_title);
      setHabitDescription(props.route.params.habit.habit_description);
    }
  }, [props.route]);
  ...
  const addHabit = async () => {
    ...
    try {
      const res = await dispatch(addCustomHabit(habitData)).unwrap();

      console.log('Habit added successfully:', res);
